...
LBDNS=$(kubectl get services spark-master --output json | jq -r '.status.loadBalancer.ingress[0].hostname')
...
kubectl delete service spark-master 2>/dev/null >/dev/null # raises unnecessary error if service doesn't exist
