...
[ -z "$AMI" ] && export AMI=$(aws ec2 describe-images --filters Name='name',Values='EKS-HermesPeakWorker-1'|jq -r '.Images[].ImageId')
#test -z "$AMI" && export AMI=$(aws ec2 describe-images --filters Name='name',Values='eks-worker-v20'|jq -r '.Images[].ImageId')  # DEfault AWS EKS working


for envvar in "VPC_STACK_NAME" "AWSACCT" "INSTANCE_TYPE" "MAXNODES" "NODESDESIRED" "AMI" "KEYNAME" "REGION"
do
	[ -z ${!envvar} ] && errorexit "${envvar} is not set"
done

# CHECK THE CLUSTER PARAMETERS ARE VALID
# CHECK THE KEYNAME
ISKEYNAME=$(aws ec2 describe-key-pairs|jq -r ".KeyPairs[] | select(.KeyName == \"$KEYNAME\") | .KeyName")
[ -z "$ISKEYNAME" ] && errorexit "Invalid KEYNAME $KEYNAME"
...
SECURITY_GROUP_EKS=$(aws ec2 describe-security-groups|jq -r '.SecurityGroups[] | select(.GroupName | contains("EFS-Group1")) | .GroupId')
...
aws cloudformation create-stack --stack-name $CLUSTER_NAME-workers \
    --template-body file://spark-eks-nodegroup.yaml \
